...
/*   Updated: 2023/06/29 15:08:39 by tlukanie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	set_color(int32_t iter, int option)
{
	...
	{
		color.a = 255;
		color.b = (uint8_t)(sin(0.2 * iter) * 67 + 150);
		color.g = (uint8_t)(sin(0.3 * iter + 2) * 97 + 100);
		color.r = (uint8_t)(sin(0.3 * iter + 1) * 127 + 60);
	}
	return (color.rgba);
}
